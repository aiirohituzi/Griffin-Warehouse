  ...
  <div class="container-expCalc">
    ...
    <div id="tdoll" class="tab-exp">
      ...
      <div class="radioCheckGroup">
        <span>목표 레벨</span>
        <input type="radio" class="radio" v-model="calcMode" value="level" />
        <span>목표 경험치</span>
        <input type="radio" class="radio" v-model="calcMode" value="exp" />
      </div>
      ...
      <div class="radioCheckGroup">
        <span title="1.2배">리더</span>
        <input type="checkbox" title="1.2배" v-model="leader" />
        <span title="1.3배">MVP</span>
        <input type="checkbox" title="1.3배" v-model="mvp" />
        <span title="개조 이후 2배">서약 여부</span>
        <input type="checkbox" title="개조 이후 2배" v-model="pledge" />
        <span>지휘요정 발동</span>
        <input type="checkbox" v-model="command" />
        <span>경험치 이벤트</span>
      ...
      <div
        style="font-size: 8pt;"
        v-if="area_selected == DIRECT_INPUT"
      >※계산 효율을 위해 입수경험치 100 미만의 경우에는 계산하지 않습니다.</div>
    ...
    <div id="div-penalty" class="side-right-exp">
      ...
      <div class="div-penaltyExplanation">
        페널티 레벨을 초과할 시, 10레벨 당 획득치 20% 감소
        <br />50레벨이상 높을 경우, 획득경험치 10으로 고정
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    expCalc: function() {
      ...
      var cumulativeExp =
        this.exp[this.tdollCurrentLv - 1] + parseInt(this.tdollCurrentExp);
      var tdollTargetExp;
      var needExp;
      var needCount = 0;
      ...
      if (this.area_selected == this.DIRECT_INPUT) {
        if (
          ...
          this.area[this.area_selected].exp < 100
        ) {
          return;
        }
      }
      ...
      var fairyNeedExp =
        this.exp_fairy[this.fairyTarget - 1] -
        (this.exp_fairy[this.fairyCurrentLv - 1] +
          parseInt(this.fairyCurrentExp));
      ...
      var FSTNeedExp =
        this.exp_FST[this.FSTTarget - 1] -
        (this.exp_FST[this.FSTCurrentLv - 1] + parseInt(this.FSTCurrentExp));
      if (isNaN(FSTNeedExp)) {
        this.FSTNeedExp = 0;
      } else {
      ...
      if (!this.pledge) {
        ...
        tdollNeedReport = Math.ceil(
          (this.exp_pledge[this.tdollTarget - 1] -
            (this.exp_pledge[this.tdollCurrentLv - 1] +
              parseInt(this.tdollCurrentExp) / 2)) /
            3000
        );
      ...
      for (needCount = 0; cumulativeExp < tdollTargetExp; needCount++) {
        ...
        if (this.command) {
          this.commandCoefficient =
            1 + this.commandSkill[this.commandSkillLv - 1] / 100;
        } else {
        ...
        cumulativeExp +=
          this.getExpFinal(cumulativeExp, penaltyLv) *
          this.pledgeCoefficient *
          this.mvpCoefficient *
          this.leaderCoefficient *
          this.commandCoefficient *
          eventCoefficient;
      }
    ...
    getExpFinal: function(cumulativeExp, penaltyLv) {
      if (this.getPenalty(cumulativeExp, penaltyLv) == 0) {
        ...
        return (
          this.getExp *
          this.getPenalty(cumulativeExp, penaltyLv) *
          this.getDummy(cumulativeExp)
        );
      }
    },
    ...
    checkInputExp: function(event) {
      ...
      if (!reg2.test(this.tdollCurrentLv)) {
        // 숫자를 입력받아도 범위가 제한된 값이라 예외처리를 따로 구분
